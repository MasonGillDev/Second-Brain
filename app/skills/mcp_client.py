...
class MCPClient:
    ...
    def _coerce_arguments(self, tool_name: str, arguments: dict) -> dict:
        """Coerce string arguments to match the tool's input_schema types.
        LLMs sometimes send 'false' as a string instead of a JSON boolean."""
        if not self._tools_cache:
            return arguments
        schema = None
        for tool in self._tools_cache:
            if tool["name"].endswith(f"__{tool_name}"):
        ...
        for key, val in coerced.items():
            ...
            # Handle anyOf (e.g. bool | None)
            if not prop_type and "anyOf" in prop:
                for option in prop["anyOf"]:
                    if option.get("type") in ("boolean", "integer", "number"):
                        prop_type = option["type"]
                        break
            if prop_type == "boolean":
                coerced[key] = val.lower().strip() not in ("false", "0", "no", "off", "none", "")
