...
async def cost_summary():
    ...
    data = await asyncio.to_thread(db.get_cost_summary, since=since, until=until, group_by=group_by)
    return jsonify(data)
...
async def cost_calls():
    ...
    calls = await asyncio.to_thread(db.get_api_calls, since=since, until=until, source=source, limit=limit, offset=offset)
    total = await asyncio.to_thread(db.get_api_calls_count, since=since, until=until, source=source)
...
async def cost_by_source():
    ...
    since = _parse_date(request.args.get("since"))
    until = _parse_date_end(request.args.get("until"))
    sources = await asyncio.to_thread(db.get_cost_by_source, since=since, until=until)
